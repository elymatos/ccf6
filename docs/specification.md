...
| Lesion and stimulation | Establish causal role | Matched intervention effects | Partial: Output Lesion and matched detector perturbations conform; candidate stimulation is unsupported | `test_lesion_suppresses_output_but_preserves_incoming_activity` |
...
| `ncl-functional-web-v1` artifacts | Make claims reproducible and inspectable | Complete file inventory | Partial: dataset, topology, arms, Presentations, learning, basins, frozen completion/reactivation evaluation, Functional Web evidence, Recruitment, homeostasis, and activity slices exist | `test_functional_web_detection_preserves_every_control_and_selected_membership` |
