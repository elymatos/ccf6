...
Second entry for the same day. The foundation ran. This is what happened when I went
back and looked at its input code. ADR-0009.

## What was wrong

The boundary showed the network **contrast**, how much each cell differs from its eight
neighbours, rather than colour. The premise was that a framework meant to record
...
Both halves were carrying weight, and both were wrong.
...
The full-neighbourhood divisor, added on 7 September, removed the border inflation, and I
took that for a fix. It was not. One T at its 80 fitting origins in a 12×12 World has
**nine distinct contrast signatures**, and only 48 origins see the interior one.

**It was also the drive magnitude, so that non-invariance reached every Space at once.**
...
This is the problem the first substrate recorded on 7 September, "strength and identity
are confused", carried into the foundation and never looked at again.
...
foreground mask instead of a contrast patch, a mask rather than colour indices, because a
raw index would drive one colour seven times harder than another and bring back the same
confusion. `World.contrast()` survives as analysis and nothing in the encoding path calls
it.
...
twitches. The run needed a number that still discriminates: **figure separation**, the
...
**The local-feature shortcut survived, as ADR-0009 said it would.** The form Space still
separates the four figures best, and it does so at its *lowest* Level. 0.148 at L1 falls
to 0.050 by L3. Depth is losing the distinction rather than building it. The shortcut
...
contrast-as-strength and nothing else. Honest rather than broken, but it means the
confusion set exercises one Space and no more.
...
rule has to beat, and it is now a number a rule could beat without the metric flattering
it.

## What this cost

The claim that CCF6 shows the network relations rather than things has moved rather than
gone. A Relation between two sampled World positions is now the only relational quantity
supplied at the boundary. Relations *within* a neighbourhood are no longer supplied at
all, and if they are wanted they have to be learned. For a project whose next spec is a
learning rule, that is where they belong.
